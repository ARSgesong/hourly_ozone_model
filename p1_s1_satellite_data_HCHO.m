...
pFilePath = '....'; % The filepath to store the GEMS satellite HCHO products
pSaveDatapath = '....'; % The filepath to store the processed L2 HCHO datasets
...
for i = 1:pf 
    ...
    swaths.hchocolumn = ncread([pFilePath Filelist(i).name],'/Data Fields/ColumnAmount');
%     swaths.hchocolumn(swaths.hchocolumn<0) = nan;
    swaths.hchocolumn(swaths.hchocolumn>1e20) = nan;
    swaths.hchocolumnuncertainty = ncread([pFilePath Filelist(i).name],'/Data Fields/ColumnUncertainty');
...
%     swaths.hchocolumn = swaths.hchocolumn(is_lat,:);
%     swaths.hchocolumnuncertainty = swaths.hchocolumnuncertainty(is_lat,:);
...
%     end
     ...
     data.hchocolumn=nan(grid_rows,grid_cols);
     data.hchocolumnuncertainty=nan(grid_rows,grid_cols);
     for r = 1:grid_rows
      for c = 1:grid_cols
        switch_ridx = grid.swaths_ridx(r,c);
        switch_cidx = grid.swaths_cidx(r,c);
                                                                                                                                                                                                                                  if ~isnan(switch_ridx) && ~isnan(switch_cidx)
            data.GroudArea(r,c) = swaths.area(switch_ridx,switch_cidx);
            data.hchocolumn(r,c) = swaths.hchocolumn(switch_ridx,switch_cidx);
            data.hchocolumnuncertainty(r,c) = swaths.hchocolumnuncertainty(switch_ridx,switch_cidx);
        end
      end
     end
