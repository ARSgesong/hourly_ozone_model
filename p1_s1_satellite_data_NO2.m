...
for i = 2926:pf
    ...
    swaths.no2column = ncread([pFilePath Filelist(i).name],'/Data Fields/ColumnAmountNO2');
    swaths.o3column = ncread([pFilePath Filelist(i).name],'/Data Fields/ColumnAmountO3');
    ...
    swaths.sza = ncread([pFilePath Filelist(i).name],'/Geolocation Fields/SolarZenithAngle');
    swaths.Slantno2column = ncread([pFilePath Filelist(i).name],'/Data Fields/SlantColumnAmountNO2');

%     is_lat = min(swaths.Latitude,[],2) <= GRID_LAT_RANGE(2)+3 & max(swaths.Latitude,[],2) >= GRID_LAT_RANGE(1)-3;
%     swaths.no2column = swaths.no2column(is_lat,:);
%     swaths.o3column = swaths.o3column(is_lat,:);
%     swaths.sza = swaths.sza(is_lat,:);
%     swaths.Latitude = swaths.Latitude(is_lat,:);
%     swaths.Longitude = swaths.Longitude(is_lat,:);
%     swaths.Slantno2column = swaths.Slantno2column(is_lat,:);
...
%     end
     ...
     data.no2column=nan(grid_rows,grid_cols);
     data.o3column=nan(grid_rows,grid_cols);
     data.sza=nan(grid_rows,grid_cols);
     for r = 1:grid_rows
      for c = 1:grid_cols
        switch_ridx = grid.swaths_ridx(r,c);
        switch_cidx = grid.swaths_cidx(r,c);
                                                                                                                                                                                                                                  if ~isnan(switch_ridx) && ~isnan(switch_cidx)
            data.GroudArea(r,c) = swaths.area(switch_ridx,switch_cidx);
            data.no2column(r,c) = swaths.no2column(switch_ridx,switch_cidx);
            data.o3column(r,c) = swaths.o3column(switch_ridx,switch_cidx);
            data.sza(r,c) = swaths.sza(switch_ridx,switch_cidx);
            data.Slantno2column(r,c) = swaths.Slantno2column(switch_ridx,switch_cidx);
